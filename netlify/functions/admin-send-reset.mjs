...
export default async function handler(request) {
  try {
    ...
    if (!student.active) throw new HttpError(409, 'Activate the student before sending a setup link.');
    const token = randomToken();
    const tokenRows = await insert('reset_tokens', { student_id: student.id, token_hash: sha256(token), expires_at: expiresIn(15 * 60) }, 'id');
    ...
    await sendEmail({ to: student.email, ...mail, idempotencyKey: `admin-reset-${tokenRows[0].id}` });
    await insert('audit_events', { student_id: student.id, event_type: 'reset_requested', details: { delivery: 'sent', requested_by: 'admin' } }, 'id');
    await insert('admin_audit_events', { admin_id: admin.id, event_type: 'student_reset_email_sent', target_student_id: student.id, details: {} }, 'id');
