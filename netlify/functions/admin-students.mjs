...
function cleanProfile(body) {
  ...
  return { email: normalizeEmail(body.email), firstName, lastName, age, phone, program, yearLevel };
}
...
export default async function handler(request) {
  try {
    ...
    if (request.method === 'POST') {
      ...
      try {
        result = await supabase('rpc/admin_create_demo_student', {
          method: 'POST',
          body: JSON.stringify({ p_student_number: studentNumber, p_email: profile.email, p_first_name: profile.firstName, p_last_name: profile.lastName, p_age: profile.age, p_phone: profile.phone, p_program: profile.program, p_year_level: profile.yearLevel, p_temporary_password: temporaryPassword })
        });
      } catch (error) {
      ...
      const mail = studentWelcomeEmail({ firstName: profile.firstName, studentNumber, temporaryPassword, signInLink: `${origin}/` });
      let emailSent = false;
      try {
        await sendEmail({ to: profile.email, ...mail, idempotencyKey: `student-welcome-${student.id}` });
        emailSent = true;
      } catch (emailError) {
        await insert('admin_audit_events', { admin_id: admin.id, event_type: 'student_welcome_email_failed', target_student_id: student.id, details: { reason: 'delivery_error' } }, 'id');
      }
      await insert('admin_audit_events', { admin_id: admin.id, event_type: 'student_created', target_student_id: student.id, details: { student_number: studentNumber, welcome_email: emailSent ? 'sent' : 'failed' } }, 'id');
      await insert('audit_events', { student_id: student.id, event_type: 'student_account_created', details: { welcome_email: emailSent ? 'sent' : 'failed', temporary_password_expires_hours: 24 } }, 'id');
      return json({
    ...
    if (request.method === 'PATCH') {
      ...
      let rows;
      try {
        rows = await update('demo_students', { id: `eq.${body.id}` }, { email: profile.email, first_name: profile.firstName, last_name: profile.lastName, age: profile.age, phone: profile.phone, program: profile.program, year_level: profile.yearLevel, active }, fields);
      } catch (error) {
      ...
      await insert('admin_audit_events', { admin_id: admin.id, event_type: active ? 'student_profile_updated' : 'student_deactivated', target_student_id: body.id, details: { fields: ['email', 'name', 'age', 'phone', 'program', 'year_level', 'active'] } }, 'id');
