...
export default async function handler(request) {
  try {
    ...
    const result = await supabase('rpc/complete_password_reset', {
      method: 'POST',
      body: JSON.stringify({ p_grant_hash: sha256(grantToken), p_password: password })
    });
    ...
    try {
      await sendEmail({ to: student.email, ...mail, idempotencyKey: `changed-${student.event_id}` });
    } catch (emailError) {
      console.error('Password changed; confirmation email failed:', emailError instanceof Error ? emailError.message : 'Unknown error');
      await insert('audit_events', { student_id: student.student_id, event_type: 'confirmation_email_failed', details: {} }, 'id');
    }
